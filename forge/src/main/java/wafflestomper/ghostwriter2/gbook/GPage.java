...
public class GPage {
	...
	public List<GLine> lines = new ArrayList();
	...
	public String addText(int lineNum, int charPos, String strAdd){
		...
		if (lineNum == this.lines.size()){
			this.lines.add(new GLine());
		}
		
		GLine currLine = this.lines.get(lineNum);
	...
	public static GPage pad(GPage page){
		GLine newLine;
		GLine lastLine = page.lines.get(page.lines.size()-1);
		if (GLine.getStringWidth(lastLine.wrappedFormatting + lastLine.text) < 116){
			lastLine.text += "\n";
		}
